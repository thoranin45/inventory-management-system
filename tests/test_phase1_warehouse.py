...
def test_warehouse_can_run_and_cancel_transfers(
    ...
    transfer._dispatch(client, warehouse_headers, draft["id"])
    transfer._receive(client, warehouse_headers, draft["id"], transfer._receive_lines(draft))
    movements = db_session.query(InventoryMovement).filter_by(
        reference_type="INVENTORY_TRANSFER", reference_id=draft["id"],
    ).all()
    # source -> transit at dispatch, transit -> destination at receipt.
